...
exports.getShowsForYear = (year) => {
  const options = {
    json: true,
    uri: `${baseUrl}/shows/query?apikey=${apikey}&year=${year}&order=DESC`,
  };

  return request.post(options).then((result) => {
    const shows = result.response.data;
    const showDates = [];

    shows.forEach((show) => {
      if (show.billed_as === 'Phish') {
        showDates.push(show.showdate);
      }
    });

    return showDates.sort();
  });
};
