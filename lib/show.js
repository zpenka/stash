...
const findOrCreateSongs = (show) => {
  ...
  const createSong = (showSong) => {
    const identifier = showSong.title;
  ...
  const songs = Object.values(show.setlist);
  return P.each(songs, (setOfSongs) => {
...
const findOrCreateSongPerformances = (show) => {
  const createSongPerformance = (songIdentifier, setIdentifier, songNumber, showDate, url) => {
  ...
  return P.each(setIdentifiers, (setIdentifier) => {
    const songs = show.setlist[setIdentifier];

    return P.each(songs, (song) => {
      const songNumber = songs.indexOf(song) + 1;
      return createSongPerformance(song.title, setIdentifier, songNumber, showDate, song.url)
      .catch((err) => {
